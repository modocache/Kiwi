...
- (void)testItShouldStubInstanceMethodsReturningObjects {
    ...
    XCTAssertEquals(fighter, [cruiser fighterWithCallsign:fighterCallsign], @"expected method to be stubbed");
}
...
- (void)testItShouldStubInstanceMethodsReturningObjectsWithAnyArguments {
    ...
    XCTAssertEquals(fighter, [cruiser fighterWithCallsign:@"Foo"], @"expected method to be stubbed");
}
...
- (void)testItShouldClearStubbedRecursiveMethods {
    ...
    [cruiser clearStubs];
    XCTAssertEquals([cruiser computeStarHashForKey:8], starHash, @"expected method to be unmodified after unstubbing");
}
...
- (void)testItShouldSubstituteMethodImplementationWithBlock {
    ...
    XCTAssertEquals(shieldsRaised, YES, @"expected method implementation to be substituted");
}
...
- (void)testItShouldPreserveClassResultWhenInstanceMethodStubbed {
    ...
    XCTAssertEquals([subject class], originalClass, @"expected class to be preserved");
}
...
- (void)testItShouldPreserveSuperclassResultWhenInstanceMethodStubbed {
    ...
    [subject stub:@selector(raiseShields) andReturn:[KWValue valueWithBool:YES]];
    XCTAssertEquals([subject superclass], originalSuperclass, @"expected superclass to be preserved");
}
...
- (void)testItShouldPreserveClassResultWhenClassMethodStubbed {
    ...
    XCTAssertEquals([Cruiser class], originalClass, @"expected class to be preserved");
}

- (void)testItShouldPreserveSuperclassResultWhenClassMethodStubbed {
    Class originalSuperclass = [Cruiser superclass];
    [Cruiser stub:@selector(classification) andReturn:@"animal"];
    XCTAssertEquals([Cruiser superclass], originalSuperclass, @"expected superclass to be preserved");
}
...
- (void)testItShouldStubAndUnstubAlloc {
    ...
    XCTAssertEquals(mock, cruiser, @"expected alloc to be stubbed");
    [Cruiser clearStubs];
...
- (void)testItShouldStubInit {
    ...
    XCTAssertEquals(cruiser, otherCruiser, @"expected init to be stubbed");
}

- (void)testSpyWorksOnRealInterfaces {
    Cruiser *cruiser = [Cruiser mock];
    XCTAssertNoThrow([cruiser captureArgument:@selector(foo) atIndex:0], @"expected not to throw exception");
}

- (void)testCallingCaptureArgumentOnRealObjectThrowsException {
    Cruiser *cruiser = [Cruiser cruiser];
    XCTAssertThrows([cruiser captureArgument:@selector(foo) atIndex:0], @"expected to throw exception");
}
...
- (void)testItShouldStubWithBlock {
    ...
    XCTAssertEquals([cruiser classification], @"Enterprise", @"expected method to be stubbed with block");
}
