...
- (void)testShouldPassTheSubjectInAsTheFirstBlockArgument
{
    ...
    XCTAssertEquals(@"subject", blockSubject, @"expected subject to be passed into matcher block");
}
...
- (void)testShouldSetTheSelectorForTheMatcher
{
    ...
    KWUserDefinedMatcher *matcher = [builder buildMatcherWithSubject:@"foo"];
    XCTAssertEquals(NSSelectorFromString(@"dummySelector"), matcher.selector, @"should set selector");
