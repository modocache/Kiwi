...
- (void)testItShouldWriteWrappedInvocationReturnValues {
    ...
    XCTAssertEquals(crewComplement, 42u, @"expected stub to write return value");
}
...
- (void)testItShouldPerformStubbedBlock {
    ...
    XCTAssertEquals(result.callsign, @"Red Leader", @"expected stub to perform given block");
}
...
- (void)testItShouldPerformStubbedBlockWhenInvocationHasNilArguments {
    ...
    XCTAssertEquals([result objectAtIndex:0], [NSNull null], @"expected stub convert nil arguments to NSNull");
}
...
- (void)testItShouldRetainValueWhenProcessingInvocationsThatBeginsWithAlloc {
    ...
    XCTAssertEquals(retainCountAfter, retainCountBefore + 1, @"expected stub to retain value");
}
