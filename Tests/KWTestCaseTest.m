...
@interface KWTestCaseTest : XCTestCase
...
- (void)testItShouldClearStubsAfterExamplesRun {
    ...
    XCTAssertEquals([subject crewComplement], 42u, @"expected method to be stubbed");

    [testCase tearDownExampleEnvironment];

    XCTAssertEquals([subject crewComplement], crewComplement, @"expected method stub to be cleared after examples run");
}
...
- (void)testItShouldNotifyVerifiersOfEndOfExample {
    ...
    [testCase invokeTest];
    XCTAssertTrue([verifier notifiedOfEndOfExample], @"expected spec to notify end of example verifiers");
}
